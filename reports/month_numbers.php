...
<table class="results">
    ...
    <tr>
        <th class="row_heading nowrap hal"><?php $morning = 0;
                    ...
                    <?php foreach ($rooms as $row_rooms) { ?>
        <td class="hac"><?php $num = SessionNumbers(date('d-m-Y',$date), 'A', $row_rooms['id']);
                            $morning += $num;
                            echo FormatTotals($num, $row_rooms['total']); ?></td>
        <td class="hac"><?php $num = SessionNumbers(date('d-m-Y',$date), 'P', $row_rooms['id']);
                            $afternoon += $num;
                            echo FormatTotals($num, $row_rooms['total']); ?></td>
