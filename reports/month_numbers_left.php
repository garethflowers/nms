...
<?php TitleReports('Monthly Numbers Left - '.date('F Y',mktime(1,1,1,$month,1,$year))); ?>
...
<table class="results">
    ...
    <tr>
        <th class="row_heading nowrap hal"><?php $morning = 0;
                        $afternoon = 0;
                        $room_total = 0;
                        echo date('d, l',$date); ?>
        </th>
                    <?php foreach ($rooms as $row_rooms) { ?>
        <td class="hac"><?php $num = $row_rooms['total'] - SessionNumbers(date('d-m-Y',$date), 'A', $row_rooms['id']);
                            $morning += $num;
                            echo FormatTotals($num, 0, true); ?></td>
        <td class="hac"><?php $num = $row_rooms['total'] - SessionNumbers(date('d-m-Y',$date), 'P', $row_rooms['id']);
                            $afternoon += $num;
                            echo FormatTotals($num, 0, true); ?></td>
                            <?php $room_total += $row_rooms['total'];
                        } ?>
        <td class="hac"><?php echo FormatTotals($row_rooms['total'] - $morning, 0, true); ?></td>
        <td class="hac"><?php echo FormatTotals($row_rooms['total'] - $afternoon, 0, true); ?></td>
    </tr>
            <?php } else { ?>
    <tr>
