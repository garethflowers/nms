...
function FormText($id, $value, $class, $maxlength) {
    $output = sprintf('<input type="text" id="%s" name="%s" value="%s" %s%s%s />', $id, $id, $value, intval($maxlength) > 0 ? sprintf(' maxlength="%s"', intval($maxlength)) : '', intval($maxlength) < 30 ? sprintf(' size="%s"', ceil(intval($maxlength) * 1.5)) : ' size="40"', $class != '' ? sprintf(' class="%s"', $class) : ''
    );
...
function FormPassword($id, $value, $class, $maxlength) {
    $output = sprintf('<input type="password" id="%s" name="%s" value="%s" %s%s%s />', $id, $id, $value, intval($maxlength) > 0 ? sprintf(' maxlength="%s"', intval($maxlength)) : '', intval($maxlength) < 30 ? sprintf(' size="%s"', ceil(intval($maxlength) * 1.25)) : '', $class != '' ? sprintf(' class="%s"', $class) : ''
    );
    $output .= sprintf(' <span id="validadvice%s"></span>', $id);
...
function FormCheck($id, $key, $value, $class) {
    $output = sprintf('<input type="checkbox" name="%s" id="%s" value="t" %s %s />', $id, $id, $class != '' ? sprintf(' class="%s"', $class) : '', $key ? ' checked="checked"' : ''
    );
...
function FormRadio($id, $values, $selected, $class, $break = false) {
    ...
    foreach ($values as $key => $value) {
        ...
        $output .= sprintf('<input type="radio" name="%s" id="%s" value="%s" %s %s /> <label for="%s">%s</label>', $id, $id . '-' . $key, $key, $class != '' ? 'class="' . $class . '"' : '', $selected == $key ? 'checked="checked"' : '', $id . '-' . $key, $value
        );
    }
...
function FormTextArea($id, $value, $class, $rows = 0) {
    $output = sprintf('<textarea id="%s" name="%s" cols="60" %s %s>%s</textarea>', $id, $id, $rows != 0 ? 'rows="' . $rows . '"' : '', $class != '' ? 'class="' . $class . '"' : '', $value
    );
    $output .= sprintf(' <span id="validadvice%s"></span>', $id);
