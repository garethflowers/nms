    ...
    <tr style="vertical-align:top">
        ...
        <td>
            ...
            <table class="results">
                ...
                <?php
                $data = Db::GetDataArray('select * from rooms order by min ASC');
                foreach ($data as $row) {
                    ?>
                    <tr class="hac">
                        <th rowspan="2">
            ...
            <ul>
                <li><?php
                $query = 'select count(id) from child where start<=now() and (finish>=now() or finish is null);';
                echo 'Total Current Children : <a href="/child">' . Db::GetScalar($query) . '</a>';
                ?></li>
                <li><?php
                    $query = 'select count(id) from staff where start<=now() and (finish>=now() or finish is null);';
                    echo 'Total Staff Members : <a href="/staff">' . Db::GetScalar($query) . '</a>';
                ?></li>
            </ul>
        </td>
    </tr>
