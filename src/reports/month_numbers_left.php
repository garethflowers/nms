        ...
        if ((date('w', $date) == 0) || (date('w', $date) == 6)) {
            ...
            echo date('d, l', $date);
                ?>
                    ...
                    <?php foreach ($rooms as $row_rooms) { ?>
                        <td class="hac"><?php
                $num = $row_rooms['total'] - SessionNumbers(date('d-m-Y', $date), 'A', $row_rooms['id']);
                $morning += $num;
                echo FormatTotals($num, 0, true);
                        ?></td>
                        <td class="hac"><?php
                $num = $row_rooms['total'] - SessionNumbers(date('d-m-Y', $date), 'P', $row_rooms['id']);
                $afternoon += $num;
                echo FormatTotals($num, 0, true);
                        ?></td>
                        <?php
